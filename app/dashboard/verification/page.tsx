...
const REDIRECT_REASON: Record<string, {
  ...
  pending: {
    icon: Clock, color: "text-[#d4a857]",
    bg: "bg-[#d4a857]/[0.07]", border: "border-[#d4a857]/25",
...
const STATUS_CONFIG: Record<string, {
  ...
  UNDER_REVIEW: {
    ...
    bg: "bg-yellow-500/5", border: "border-yellow-500/25",
    title: "Under Review",
    message: "Your documents are going through our verification process.",
  },
  REJECTED: {
...
export default function VerificationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Verification submitted! Your documents will be processed within 1–3 business days.");
      setStatus("PENDING");
    } catch {
  ...
  /* ── Shared input class ────────────────────────────────────────────────────────── */
  const inputCls = "bg-white/[0.06] border-white/[0.18] text-white placeholder:text-slate-500 h-10 focus:border-[#d4a857]/70 focus:bg-white/[0.08] hover:border-white/30 transition-colors";
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <Select defaultValue="Passport" onValueChange={v => v && setDocType(v)}>
                <SelectTrigger className="bg-white/[0.06] border-white/[0.18] text-white h-10 focus:border-[#d4a857]/70 hover:border-white/30 transition-colors">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#0E1A30] border-[#d4a857]/20 text-white">
                  {["Passport", "Driver's License", "National ID"].map(d => (
                    <SelectItem key={d} value={d} className="text-white hover:bg-[#d4a857]/12 focus:bg-[#d4a857]/12">
                      {d}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              {docFile ? (
                ...
                <div className="flex items-center gap-3 rounded-lg px-4 py-3
                  bg-[#d4a857]/[0.07] border border-[#d4a857]/30 transition-all">
                  <FileImage className="h-5 w-5 text-[#d4a857] flex-shrink-0" />
                ...
                <div
                  ...
                  className={`
                    ...
                    ${isDragging
                      ? "border-[#d4a857]/70 bg-[#d4a857]/[0.10] scale-[1.01]"
                      : "border-white/[0.22] hover:border-[#d4a857]/50 hover:bg-white/[0.03]"}
                  `}
                >
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 transition-colors ${isDragging ? "bg-[#d4a857]/20" : "bg-white/[0.06]"}`}>
                    <Upload className={`h-5 w-5 transition-colors ${isDragging ? "text-[#d4a857]" : "text-slate-400"}`} />
                  </div>
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full bg-[#d4a857] hover:bg-[#d4a857] text-white font-semibold h-11
                shadow-lg shadow-[#d4a857]/30 hover:shadow-[#d4a857]/50 transition-all"
            >
              {loading
