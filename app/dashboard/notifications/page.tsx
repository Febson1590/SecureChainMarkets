...
const NOTIF_COLORS: Record<string, string> = {
  INFO: "text-[#2B6BFF] bg-[#2B6BFF]/12", TRADE: "text-violet-400 bg-violet-500/10",
  DEPOSIT: "text-emerald-400 bg-emerald-500/10", WITHDRAWAL: "text-orange-400 bg-orange-500/10",
  VERIFICATION: "text-yellow-400 bg-yellow-500/10", SUPPORT: "text-[#2B6BFF] bg-[#2B6BFF]/12",
      ...
      {notifications.length === 0 ? (
        ...
        <div className="glass-card rounded-xl overflow-hidden divide-y divide-white/5">
          {notifications.map((notif) => {
            ...
            const colors = NOTIF_COLORS[notif.type] || "text-[#2B6BFF] bg-[#2B6BFF]/12";
            return (
              <div key={notif.id} className={`flex items-start gap-4 px-5 py-4 transition-colors ${!notif.isRead ? "bg-[#2B6BFF]/3" : "hover:bg-white/2"}`}>
