    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          size="sm"
          className="bg-[#d4a857] hover:bg-[#d4a857] text-white text-xs"
        >
      ...
      {view === "new" ? (
        <Card className="glass-card border-0 rounded-xl p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Select defaultValue="General" onValueChange={(v) => v !== null && setValue("category", v)}>
                ...
                <SelectContent className="bg-[#0E1A30] border-[#d4a857]/20 text-white">
                  {categories.map((c) => (
                    <SelectItem key={c} value={c} className="hover:bg-[#d4a857]/12 focus:bg-[#d4a857]/12">{c}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
              ...
              <Select defaultValue="MEDIUM" onValueChange={(v) => v !== null && setValue("priority", v)}>
                ...
                <SelectContent className="bg-[#0E1A30] border-[#d4a857]/20 text-white">
                  {["LOW", "MEDIUM", "HIGH", "URGENT"].map((p) => (
                    <SelectItem key={p} value={p} className="hover:bg-[#d4a857]/12 focus:bg-[#d4a857]/12">{p}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <Button type="submit" disabled={loading} className="w-full bg-[#d4a857] hover:bg-[#d4a857] text-white font-semibold h-11">
        ...
        <div className="space-y-5">
          ...
          <div className="glass-card rounded-xl p-5">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-lg bg-[#d4a857]/12 border border-[#d4a857]/20 flex items-center justify-center">
                <HeadphonesIcon className="h-5 w-5 text-[#d4a857]" />
              </div>
              <div>
          ...
          <div className="text-center">
            ...
            <Button onClick={() => setView("new")} size="sm" className="bg-[#d4a857] hover:bg-[#d4a857] text-white">
