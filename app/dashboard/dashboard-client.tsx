  ...
  return (
    <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.05]">
      <div className="flex items-center gap-2">
        {Icon && <Icon size={14} className="text-[#d4a857]" />}
...
function AddFundsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-sm rounded-2xl p-6 shadow-2xl border border-[#d4a857]/20"
      ...
      >
        ...
        <p className="text-xs text-slate-400 mb-5">
          From <span className="text-[#e8c178] font-semibold">Deposit Balance</span>:{" "}
        ...
        <input
          ...
          autoFocus
          className="w-full bg-white/[0.05] border border-white/[0.12] rounded-lg px-4 py-3 text-white text-sm placeholder:text-slate-500 focus:outline-none focus:border-[#d4a857]/50 mb-4"
        />
        <div className="flex gap-2">
          ...
          <Button
            className="flex-1 bg-[#d4a857] hover:bg-[#d4a857] text-white font-semibold h-10"
            onClick={submit}
            disabled={loading}
          >
...
function UpgradeModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl border border-[#d4a857]/20 shadow-2xl my-4 sm:my-auto"
        style={{ background: "rgba(7,15,30,0.98)" }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left rounded-xl border px-4 py-3 transition ${
                        active
                          ? "border-[#d4a857]/60 bg-[#d4a857]/[0.08]"
                ...
                <div className="pt-2 border-t border-white/[0.06] space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.08]">
                      ...
                      <div className="text-[13px] font-semibold text-[#d4a857] tabular-nums">{fmt(selected.minAmount)}</div>
                    </div>
                    <div className="px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.08]">
        ...
        <div className="px-5 pt-5 pb-4">
          ...
          <div className="flex items-center gap-2 mt-4">
            <Link href={isKycApproved ? "/dashboard/deposit" : kycHref} className="flex-1 sm:flex-none">
              <Button className="w-full sm:w-auto h-10 px-5 bg-[#d4a857] hover:bg-[#d4a857] text-white font-semibold text-[13px]">
      ...
      {isInvestmentLive ? (
        <Card>
          ...
          {investment!.status === "ACTIVE" && (
            <div className="px-5 pb-5 flex gap-2">
              <Button
                className="flex-1 h-10 bg-[#d4a857]/[0.10] hover:bg-[#d4a857]/[0.18] text-[#e8c178] border border-[#d4a857]/25 font-semibold text-[13px]"
                onClick={() => {
              ...
              <Button
                className="flex-1 h-10 bg-[#d4a857] hover:bg-[#d4a857] text-white font-semibold text-[13px]"
                onClick={() => {
                  if (!isKycApproved) { router.push(kycHref); return; }
                  setShowUpgrade(true);
                }}
        ...
        <CardHeader
          ...
          action={
            <Link
              ...
              className="text-[12px] text-[#d4a857] hover:text-[#e8c178] flex items-center gap-0.5 font-medium"
            >
...
function QuickAction({
  ...
  return (
    <Link
      href={href}
      className="flex-shrink-0 flex items-center gap-2 h-10 px-4 rounded-full border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.05] hover:border-[#d4a857]/25 transition-colors"
    >
      <Icon size={14} className="text-[#d4a857]" />
...
function KycBanner({ kycStatus }: { kycStatus: "not_submitted" | "pending" | "approved" | "rejected" }) {
  ...
  if (kycStatus === "pending") {
    return (
      <div className="rounded-xl px-4 py-3 flex items-center justify-between gap-4 border border-[#d4a857]/20"
        style={{ background: "rgba(14,165,233,0.05)" }}
      >
        <div className="flex items-center gap-3">
          <Clock className="h-5 w-5 text-[#d4a857] flex-shrink-0" />
          <div>
            <p className="text-[13px] font-semibold text-[#e8c178]">Verification Under Review</p>
            <p className="text-[11.5px] text-slate-400 mt-0.5">Your documents are being reviewed. We&apos;ll notify you once complete.</p>
          </div>
        </div>
        <Link href="/dashboard/verification" className="flex-shrink-0">
          <Button size="sm" variant="outline" className="border-[#d4a857]/30 text-[#e8c178] hover:bg-[#d4a857]/12 font-semibold text-xs h-8 px-3">
            View Status
          </Button>
        </Link>
      </div>
