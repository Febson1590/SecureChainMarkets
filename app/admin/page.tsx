...
export default async function AdminDashboard() {
  ...
  const statCards = [
    { label: "Total Users", value: stats.totalUsers, sub: `${stats.activeUsers} active`, icon: Users, color: "text-[#d4a857]" },
    ...
    { label: "Active Users", value: stats.activeUsers, sub: `of ${stats.totalUsers} total`, icon: Activity, color: "text-[#d4a857]" },
  ];
            ...
            {recentUsers.map((user) => (
              <div key={user.id} className="flex items-center justify-between px-4 py-3 hover:bg-white/2 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-[#d4a857]/12 border border-[#d4a857]/20 flex items-center justify-center text-xs font-bold text-[#d4a857]">
            ...
            {recentActions.length === 0 ? (
              ...
              <div key={action.id} className="px-4 py-3 hover:bg-white/2 transition-colors">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-[#d4a857]">{action.action}</span>
