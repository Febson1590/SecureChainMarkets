...
export default function AdminSupportPage() {
  ...
  const PRIORITY_COLORS: Record<string, string> = {
    ...
    MEDIUM: "text-[#D4AF37] border-[#D4AF37]/20 bg-[#D4AF37]/12",
            ...
            <div key={ticket.id} className="glass-card rounded-xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-9 h-9 rounded-full bg-[#D4AF37]/12 border border-[#D4AF37]/20 flex items-center justify-center text-xs font-bold text-[#D4AF37] flex-shrink-0">
                      ...
                      ticket.messages?.map((msg: any) => (
                        <div key={msg.id} className={`flex ${msg.senderRole === "ADMIN" ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[80%] rounded-xl px-4 py-2.5 ${msg.senderRole === "ADMIN"
                            ? "bg-[#D4AF37]/15 border border-[#D4AF37]/20 text-white"
                    ...
                    <div className="p-4 border-t border-white/5 space-y-3">
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" disabled={!!sending} onClick={() => handleReply(ticket.id, false)}
                          className="bg-[#D4AF37] hover:bg-[#D4AF37] text-white h-8 px-4 text-xs">
