...
const STATUS_COLORS: Record<string, string> = {
  ...
  COMPLETED: "bg-[#D4AF37]/12 border-[#D4AF37]/25 text-[#D4AF37]",
    ...
    <div>
      ...
      <div className="mt-1 flex gap-2">
        ...
        <div className="relative shrink-0">
          ...
          >
            <option value="minutes" className="bg-[#14141A]">{UNIT_LABELS.minutes}</option>
            <option value="hours"   className="bg-[#14141A]">{UNIT_LABELS.hours}</option>
          </select>
...
function PlanModal({ plan, onClose, onSuccess }: { plan?: Plan; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="glass-card border border-[#D4AF37]/20 rounded-2xl p-5 sm:p-6 w-full max-w-md shadow-2xl my-4 sm:my-auto"
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <p className="text-[11px] text-slate-500 -mt-2 inline-flex items-center gap-1.5">
            <Info size={11} className="text-[#D4AF37]/70" />
          ...
          <label className="flex items-center gap-2 cursor-pointer mt-2">
            ...
            <span className="text-xs text-slate-300">
              Mark as <strong className="text-[#D4AF37]">Most Popular</strong>{" "}
        ...
        <div className="flex gap-2 mt-6">
          ...
          <Button className="flex-1 bg-[#D4AF37] hover:bg-[#D4AF37] text-white font-semibold" onClick={submit} disabled={loading}>
          ...
          {!isEdit && (
            <div>
              ...
              <div className="relative mt-1">
                ...
                >
                  <option value="" className="bg-[#14141A]">Select user…</option>
                  {users.map(u => (
                    <option key={u.id} value={u.id} className="bg-[#14141A]">
                      {u.name || "—"} ({u.email})
                    </option>
                  ))}
                </select>
          ...
          <div>
            ...
            <div className="relative mt-1">
              <select
                ...
                className={inputCls + " appearance-none pr-8"}
              >
                <option value="" className="bg-[#14141A]">
                  {isEdit ? "Custom (keep current values)" : "Custom — fill fields manually"}
                </option>
                {plans.filter(p => p.isActive).map(p => (
                  <option key={p.id} value={p.id} className="bg-[#14141A]">{p.name}</option>
                ))}
              </select>
              <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
            </div>
...
function AddFundsModal({ investment, onClose, onSuccess }: { investment: UserInvestment; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-start sm:items-center overflow-y-auto p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-card border border-[#D4AF37]/20 rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-4 px-3 py-2.5 rounded-lg bg-white/[0.04] border border-white/10">
          ...
          <div className="text-xs text-slate-500">{investment.user.email} · <span className="text-[#D4AF37]">{investment.planName}</span></div>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <TrendingUp size={20} className="text-[#D4AF37]" /> Investments
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage investment plans and user portfolios</p>
        </div>
        <div className="flex gap-2">
          {tab === "plans" && (
            <Button onClick={() => setShowCreatePlan(true)} className="bg-[#D4AF37] hover:bg-[#D4AF37] text-white font-semibold text-sm h-9 px-4">
              <Plus size={14} className="mr-1.5" /> New Plan
            </Button>
          )}
          {tab === "users" && (
            <Button onClick={() => setShowAssign(true)} className="bg-[#D4AF37] hover:bg-[#D4AF37] text-white font-semibold text-sm h-9 px-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Active Invs",   value: activeInv,                                                     color: "text-[#D4AF37]" },
      ...
      <div className="flex gap-1 border-b border-white/5">
        {(["plans", "users"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2.5 text-sm font-medium transition-colors capitalize ${tab === t ? "text-[#D4AF37] border-b-2 border-[#D4AF37]" : "text-slate-500 hover:text-white"}`}>
                    ...
                    <tr key={plan.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          {plan.isPopular && (
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-[#D4AF37]/15 text-[#E6C158] border border-[#D4AF37]/30">
                              POPULAR
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <Button size="sm" onClick={() => setEditPlan(plan)}
                            className="h-7 px-2 text-xs bg-[#D4AF37]/12 hover:bg-[#D4AF37]/20 text-[#D4AF37] border border-[#D4AF37]/20">
                          ...
                          {(inv.status === "ACTIVE" || inv.status === "PAUSED") && (
                            <>
                              <Button size="sm" disabled={processing === inv.userId} onClick={() => handleEndTrade(inv)}
                                ...
                                className="h-7 px-2 text-xs bg-[#D4AF37]/12 hover:bg-[#D4AF37]/20 text-[#E6C158] border border-[#D4AF37]/25">
