              ...
              <div key={v.id} className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-[#D4AF37]/12 border border-[#D4AF37]/20 flex items-center justify-center text-xs font-bold text-[#D4AF37] flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    {v.status === "PENDING" && (
                      <Button size="sm" variant="outline" onClick={() => setExpanded(expanded === v.id ? null : v.id)}
                        className="border-[#D4AF37]/20 text-[#D4AF37] hover:bg-[#D4AF37]/12 h-7 text-xs px-3">
                              ...
                              {isImageUrl(doc.url) ? (
                                <a href={doc.url} target="_blank" rel="noopener noreferrer" className="block">
                                  <img
                                    src={doc.url}
                                    alt={doc.label}
                                    className="w-full h-32 object-cover rounded border border-white/10 hover:border-[#D4AF37]/40 transition-colors"
                                  />
                                </a>
                              ) : (
                                <a href={doc.url} target="_blank" rel="noopener noreferrer" className="block">
                                  <div className="h-24 bg-white/5 rounded flex items-center justify-center text-xs text-[#D4AF37] hover:bg-white/10 transition-colors">
                                    View Document &rarr;
                                  </div>
                                </a>
                              )}
