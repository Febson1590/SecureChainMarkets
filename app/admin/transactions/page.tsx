  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        <a href="/admin/transactions" className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${!typeFilter ? "bg-[#d4a857]/20 border-[#d4a857]/40 text-[#d4a857]" : "border-white/10 text-slate-400 hover:border-white/20"}`}>All</a>
        {TX_TYPES.map((type) => (
          <a key={type} href={`/admin/transactions?type=${type}`}
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${typeFilter === type ? "bg-[#d4a857]/20 border-[#d4a857]/40 text-[#d4a857]" : "border-white/10 text-slate-400 hover:border-white/20"}`}>
            {type}
          </a>
        ))}
      </div>
