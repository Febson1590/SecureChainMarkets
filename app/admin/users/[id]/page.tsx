...
const INPUT = "w-full bg-white/[0.06] border border-white/[0.12] rounded-lg px-3 py-2 text-white text-sm placeholder:text-slate-500 focus:outline-none focus:border-[#D4AF37]/60";
...
const TX_CLR: Record<string, string> = {
  ...
  ADJUSTMENT: "bg-[#D4AF37]/12 text-[#D4AF37]",
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative mt-1.5">
                ...
                >
                  {STATUS_OPTS.map(s => (
                    <option key={s} value={s} className="bg-[#14141A]">{s}</option>
                  ))}
                </select>
            ...
            <Button
              ...
              className="w-full bg-[#D4AF37] hover:bg-[#D4AF37] text-white font-bold h-9 text-sm"
            >
              {statusBusy
              ...
              <div className="relative mt-1.5">
                <select value={nType} onChange={e => setNType(e.target.value)} className={INPUT + " appearance-none pr-8"}>
                  {["INFO", "SUCCESS", "WARNING", "ERROR"].map(t => (
                    <option key={t} value={t} className="bg-[#14141A]">{t}</option>
                  ))}
                </select>
                <svg className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg>
              </div>
      ...
      <div className={CARD}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          {CURRENCIES.map(c => (
            <button
              ...
              className={`rounded-xl p-3.5 border text-left transition-all ${selCurrency === c ? "border-[#D4AF37]/60 bg-[#D4AF37]/12 shadow-[0_0_0_1px_rgba(14,165,233,0.2)]" : "border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.06]"}`}
            >
              <div className="flex items-center justify-between mb-1.5">
                <span className={`text-[10px] font-extrabold uppercase tracking-widest ${selCurrency === c ? "text-[#D4AF37]" : "text-slate-500"}`}>{c}</span>
                {selCurrency === c && <div className="w-1.5 h-1.5 rounded-full bg-[#D4AF37]" />}
              </div>
        ...
        <div className="border-t border-white/[0.06] pt-5">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
            Adjust <span className="text-[#D4AF37]">{selCurrency}</span> Balance
          </p>
          ...
          <div className="flex flex-wrap gap-2.5">
            ...
            <Button
              ...
              className="h-9 px-5 text-sm font-bold bg-[#D4AF37]/12 hover:bg-[#D4AF37]/20 text-[#D4AF37] border border-[#D4AF37]/25"
            >
              {wOp === "SET"
                ? <Loader2 size={13} className="animate-spin mr-1.5" />
          ...
          {investment ? (
            <div className="space-y-0 divide-y divide-white/[0.05]">
              {[
                ...
                { label: "Invested",     value: `$${Number(investment.amount).toLocaleString("en-US", { minimumFractionDigits: 2 })}`, cls: "text-[#D4AF37] font-bold" },
        ...
        <div className={CARD}>
          <div className="flex items-center gap-2 mb-4">
            ...
            {activeCopies.length > 0 && (
              <span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#D4AF37]/12 border border-[#D4AF37]/20 text-[#D4AF37]">
            ...
            <div className="divide-y divide-white/[0.04]">
              {adminLog.map((a: any) => (
                <div key={a.id} className="flex items-start gap-3 py-2.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#D4AF37]/50 flex-shrink-0 mt-1.5" />
                  <div className="flex-1 min-w-0">
