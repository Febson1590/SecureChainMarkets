  ...
  return (
    ...
    >
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetTrigger render={<button className="lg:hidden p-2 text-slate-600 hover:text-[#1A1A22] mr-2" />}>
          <Menu size={20} />
        </SheetTrigger>
        <SheetContent
          side="left"
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <button
          ...
          className="p-2 text-slate-600 hover:text-[#1A1A22] hover:bg-slate-100 rounded-lg transition-colors"
        >
          <Globe size={18} />
        </button>
        ...
        <Link
          ...
          className="relative p-2 text-slate-600 hover:text-[#1A1A22] hover:bg-slate-100 rounded-lg transition-colors"
        >
          <Bell size={18} />
          {unreadCount > 0 && (
            <span className="absolute top-1 right-1 w-4 h-4 rounded-full text-[10px] font-bold text-white flex items-center justify-center" style={{ background: "linear-gradient(180deg, #D4AF37 0%, #B8941F 100%)" }}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger render={<button className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-slate-100 transition-colors" />}>
            <Avatar className="h-8 w-8">
              <AvatarFallback className="bg-[#D4AF37]/15 text-[#D4AF37] text-xs font-bold">
                {initials}
              </AvatarFallback>
            </Avatar>
            <div className="hidden sm:block text-left">
              <div className="text-sm font-medium text-[#1A1A22] leading-none mb-0.5">{user.name || "User"}</div>
              <div className="text-xs text-slate-500 leading-none">{user.role === "ADMIN" ? "Administrator" : "Trader"}</div>
            </div>
            <ChevronDown size={14} className="text-slate-500 hidden sm:block" />
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-60 bg-white border-slate-200 text-[#1A1A22] shadow-[0_18px_40px_-18px_rgba(20, 20, 26,0.18)] rounded-xl p-1.5">
            <div className="px-3 py-2.5">
              <p className="text-[13.5px] font-semibold text-[#1A1A22] truncate leading-tight">{user.name}</p>
            ...
            {user.role === "ADMIN" && (
              <>
                <DropdownMenuItem render={<Link href="/admin" />} className="hover:bg-slate-50 cursor-pointer text-[#D4AF37] font-semibold rounded-lg px-3 py-2 text-[13px]">
                  Admin Panel
                </DropdownMenuItem>
                <DropdownMenuSeparator className="bg-slate-100 my-1" />
              </>
